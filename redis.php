...
class Lib_Redis{
			...
			for($try = 0; $try < 2; $try++){		
				try{
					$this->connect = $this->persist ? 
						...
						$this->oRedis->connect((string)$this->aServer['host'], (int)$this->aServer['port'], (float)$this->timeout);
    ...
    public function hGet($key, $hashKey){
		...
		if($etime - $stime >= 1){
			...
			$dateTime = date('m-d H:i:s');
			$uri = (isset( $_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''). '--';
			$error = "[{$dateTime}] > hGet time:{$runTime}; key:{$key}; uri:{$uri}";
			//oo::logs()->debug($error, 'muredis.runtimeout.txt');
		}
		return $res;
    ...
    public function hDel($key, $hashKey){
    	for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				$res = $this->connect('hDel', $key) && $this->oRedis->hDel($key, $hashKey);
				break;
			}catch (RedisException $e){
				$this->close(); //显式关闭,强制重连
    ...
    public function hGetAll( $key){
		...
		if($etime - $stime >= 1){
			...
			$dateTime = date('m-d H:i:s');
			$uri = (isset( $_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''). '--';
			$error = $this->aServer['host'] . ':' .$this->aServer['port'] . "[{$dateTime}] > hGetAll time:{$runTime}; key:{$key}; uri:{$uri}";
			//oo::logs()->debug($error, 'muredis.runtimeout.txt');
		}
		return $res;
    ...
    public function hExists($key, $memberKey){
		...
		if($etime - $stime >= 1){
			...
			$dateTime = date('m-d H:i:s');
			$uri = (isset( $_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''). '--';
			$error = $this->aServer['host'] . ':' .$this->aServer['port'] . "[{$dateTime}] > hIncrBy time:{$runTime}; key:{$key}; uri:{$uri}";
			//oo::logs()->debug($error, 'muredis.runtimeout.txt');
		}
		return $res;
    }
    ...
    public function hIncrByFloat($key, $member, $value){
		return $this->connect('hIncrByFloat', $key) ? $this->oRedis->hIncrByFloat($key, $member, (float)$value) : false;
    }
    /**
    ...
    public function hMget($key, $memberKeys){
		...
		if($etime - $stime >= 1){
			...
			$dateTime = date('m-d H:i:s');
			$uri = (isset( $_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : ''). '--';
			$error = $this->aServer['host'] . ':' .$this->aServer['port'] . "[{$dateTime}] > hMget time:{$runTime}; key:{$key}; uri:{$uri}";
			//oo::logs()->debug($error, 'muredis.runtimeout.txt');
		}
		return $res;
	...
	private function errorlog($keys, $code, $msg, $die=false, $file = 'muredis.txt'){
		...
		$host = (string)$this->aServer['host'];
		$port = (int)$this->aServer['port'];
